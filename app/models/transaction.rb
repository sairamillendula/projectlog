class Transaction < ActiveRecord::Base
  ...
  scope :by_keyword, lambda {|keyword| where('(transactions.note LIKE ?) OR (transactions.total = ?)', "%#{keyword}%", keyword.gsub(/,/, '.').to_f)}
    ...
    def to_csv(transactions, options={})
      CSV.generate(options) do |csv|
        ...
        transactions.each do |transaction|
          csv << [transaction.date.to_date, transaction.expense? ? 'Expense' : 'Income', transaction.total, transaction.tax1, transaction.tax2, transaction.note, transaction.category.present? ? transaction.category.name : nil , transaction.project.present? ? transaction.project.title : nil]
        end
      end
    end
  end
